...
const services = [
  ...
  },
  {
    ...
    icon: <FaGlobe size={40} className="text-purple-400" />,
  },
  {
    title: "IT Consulting",
    description:
  ...
  },
  {
    ...
    icon: <FaSync size={40} className="text-pink-400" />,
  },
  {
    ...
    icon: <FaCloud size={40} className="text-indigo-400" />,
  },
];
  ...
  return (
    <div className="w-full py-10 bg-white">
      <div className="max-w-7xl mx-auto p-4">
        ...
        <p className="text-lg text-start text-slate-500  mb-10">We prioritize delivering outstanding solutions, comprehensive training, and seamless execution
        to ensure success.</p>
        <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-7">
          {services.map((service, index) => (
            <div key={index} className="w-full h-[250px] rounded-2xl shadow-reviwcard p-5 flex flex-col gap-3 mb-10">
              <div className="rounded-lg  cursor-pointer transition-all duration-500 group-hover:bg-indigo-600 mb-2">
                {service.icon}
              </div>
              <h4 className="text-lg font-bold text-gray-900  capitalize">{service.title}</h4>
              <p className="text-md font-normal text-gray-400">{service.description}</p>
            </div>
          ))}
        </div>
      </div>
